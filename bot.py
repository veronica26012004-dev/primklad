...
def get_items_file(storage):
    ...
    if not storage_id:
        print(f"Ошибка: неверный storage {storage}, доступные: {list(STORAGE_IDS.keys())}")
        return None
...
def load_items(storage):
    file_path = get_items_file(storage)
    if not file_path:
        print(f"Ошибка: не удалось определить файл для {storage}")
        return []
    with db_lock:
        try:
            items = []
                        ...
                        if line:
                            ...
                            if len(parts) == 3:
                                ...
                                try:
                                    ...
                                    print(f"Неверный формат issued в {file_path}: {line}")
                            else:
                                print(f"Неверный формат строки в {file_path}: {line}")
            else:
                print(f"Файл не существует: {file_path}")
            return items
        except Exception as e:
            print(f"Ошибка загрузки предметов из {file_path}: {e}")
            return []
...
def save_items(items, storage):
    ...
    with db_lock:
        try:
            ...
            print(f"Ошибка сохранения предметов в {file_path}: {e}")
            return False
...
def get_inventory(storage):
    try:
        ...
        print(f"Ошибка получения инвентаря для {storage}: {e}")
        return []
...
def add_item(item_name, storage):
    ...
    try:
        ...
        print(f"Ошибка добавления предмета {item_name} в {storage}: {e}")
        return None
...
def delete_items(item_names, storage):
    try:
        ...
        print(f"Ошибка удаления предметов из {storage}: {e}")
        return []
...
def update_items_owner(item_names, owner, storage):
    try:
        ...
        print(f"Ошибка обновления статуса предметов в {storage}: {e}")
        return []
...
def return_items(item_names, storage):
    try:
        ...
        print(f"Ошибка возврата предметов в {storage}: {e}")
        return []
...
def find_item_in_db(item_name, storage):
    try:
        ...
        print(f"Ошибка поиска предмета {item_name} в {storage}: {e}")
        return None
...
def add_event(event_name, event_date):
    ...
    try:
        ...
        print(f"Ошибка добавления события {event_name}: {e}")
        return None
...
def delete_event(event_ids):
    try:
        ...
        print(f"Ошибка удаления событий: {e}, event_ids: {event_ids}")
        return []
...
def create_items_keyboard(chat_id, storage, action):
    if storage is None or storage not in STORAGE_IDS:
        print(f"Ошибка: неверное хранилище {storage} при создании клавиатуры")
        return None
    ...
    if chat_id in user_item_lists:
        ...
        storage_id = STORAGE_IDS.get(storage)
        if not storage_id:
            print(f"Ошибка: не удалось получить storage_id для {storage}")
            return None
...
def show_inventory(chat_id, storage):
    if storage not in STORAGE_IDS:
        print(f"Ошибка: неверная кладовая {storage} в show_inventory")
        bot.send_message(chat_id, "❌ Не удалось выбрать кладовую, попробуйте снова")
        show_storage_selection(chat_id)
        return
    file_path = get_items_file(storage)
    if not file_path or not os.path.exists(file_path):
        print(f"Ошибка: файл {file_path} не найден для {storage}")
...
def handle_storage(message):
    print(f"Переход в выбор кладовой для chat_id {message.chat.id}")
...
def back_to_storage_selection(message):
    print(f"Возврат к выбору кладовой для chat_id {message.chat.id}")
    show_storage_selection(message.chat.id)
...
def handle_storage_selection(message):
    ...
    print(f"Обработка выбора кладовой: message.text='{message.text}', chat_id={chat_id}")
    # Удаляем эмодзи 📍 из текста кнопки
    storage = message.text.replace('📍 ', '').strip()
    print(f"Нормализованная кладовая: {storage}")
    if storage in STORAGE_IDS:
        show_storage_menu(chat_id, storage)
    elif message.text == '🔙 В главное меню':
        print(f"Возврат в главное меню для chat_id {chat_id}")
        show_main_menu(chat_id)
    else:
        print(f"Неверный выбор кладовой: {message.text}")
...
def handle_storage_actions(message):
    ...
    if len(state_data) >= 2:
        ...
        print(f"Ошибка: неверное состояние для chat_id {chat_id}: {state_data}")
        bot.send_message(chat_id, "❌ Ошибка состояния, выберите кладовую снова")
        show_storage_selection(chat_id)
        return

    print(f"Обработка действия в кладовой {storage} для chat_id {chat_id}: {message.text}")
    ...
    if message.text == '➕ Добавить предмет':
        ...
        print(f"Возврат к выбору кладовой из storage для chat_id {chat_id}")
        show_storage_selection(chat_id)
...
def handle_entering_owner_name(message):
    ...
    if len(state_data) >= 3:
        ...
        print(f"Ошибка: неверное состояние entering_owner_name для chat_id {chat_id}: {state_data}")
        bot.send_message(chat_id, "❌ Ошибка состояния, выберите кладовую снова")
        show_storage_selection(chat_id)
        return
    ...
    if updated_names:
        ...
        print(f"Не удалось выдать предметы: {selected_items} для {owner}")
...
def handle_callback(call):
    ...
    print(f"Callback data: {callback_data}, chat_id: {chat_id}")
    ...
    try:
        if callback_data.startswith('select_'):
            ...
            if callback_data.startswith('select_event_'):
                ...
                if len(parts) < 4 or parts[3] != 'delete':
                    ...
                    print(f"Неверный формат callback_data для события: {callback_data}")
                    show_events_menu(chat_id)
                    return
                ...
                if not any(ev[0] == event_id for ev in events):
                    ...
                    print(f"Не найден event_id: {event_id}")
                    show_events_menu(chat_id)
                    return
                ...
                if not storage:
                    ...
                    print(f"Не найден storage_id: {storage_id} в REVERSE_STORAGE_IDS")
                    show_storage_selection(chat_id)
                    return

                if chat_id not in user_item_lists:
                    bot.answer_callback_query(call.id, "❌ Ошибка: список предметов устарел")
                    print(f"Нет user_item_lists для chat_id {chat_id}")
                    show_storage_selection(chat_id)
                    return
                ...
                if item_list_data['key'] != expected_key:
                    ...
                    print(f"Несоответствие ключей: expected {expected_key}, got {item_list_data['key']}")
                    show_storage_selection(chat_id)
                    return
                ...
                try:
                    ...
                    if item_index < 0 or item_index >= len(item_list_data['items']):
                        ...
                        print(f"Неверный item_index: {item_index}, items: {len(item_list_data['items'])}")
                        return
                    item_id, item_name, issued, owner = item_list_data['items'][item_index]
                except (ValueError, IndexError) as e:
                    bot.answer_callback_query(call.id, "❌ Ошибка: неверный индекс предмета")
                    print(f"Ошибка индекса предмета: {e}, item_index: {item_index}")
                    return
                ...
                try:
                    ...
                    if "message is not modified" not in str(e):
                        print(f"Ошибка обновления клавиатуры предметов: {e}, callback_data: {callback_data}")
            ...
            if len(parts) < 3:
                bot.answer_callback_query(call.id, "❌ Ошибка: неверный формат callback")
                print(f"Неверный формат confirm callback_data: {callback_data}")
                show_storage_selection(chat_id)
                return
            ...
            if not selected_items:
                ...
                print(f"Нет выбранных предметов для chat_id {chat_id}")
                try:
                    bot.delete_message(chat_id, call.message.message_id)
                except Exception as e:
            ...
            if action == 'delete':
                ...
                if deleted_names:
                    ...
                    print(f"Не удалось удалить предметы: {selected_items}")
                ...
                if returned_names:
                    ...
                    print(f"Не удалось вернуть предметы: {selected_items}")
            ...
            if len(parts) < 3:
                bot.answer_callback_query(call.id, "❌ Ошибка: неверный формат callback")
                print(f"Неверный формат clear callback_data: {callback_data}")
                show_storage_selection(chat_id)
                return
            action, storage_id = parts[1], parts[2]
        ...
        print(f"Общая ошибка обработки callback: {e}, callback_data: {callback_data}")
